...
class EchoPlugin(Star):
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        if tracker and tracker.alive:
            if now > tracker.expire_at:
                ...
                if self._is_probability_hit(self._get_effective_reply_prob(group_id, umo)):
                    ...
                    asyncio.create_task(self._handle_reply(tracker, event))
                return
        ...
        if self._is_probability_hit(active_prob):
            ...
            asyncio.create_task(self._handle_proactive(event, msg, window))

    async def _handle_reply(self, tracker: ConversationTracker, event: AstrMessageEvent):
    ...
    async def _handle_proactive(self, event: AstrMessageEvent, msg: dict, recent_window: List[dict]):
        group_id = str(event.get_group_id())
        try:
            gname = ""
            try:
    ...
    async def _call_generator_raw(self, prompt: str, image_urls: list = None, umo: str = "") -> Optional[str]:
        ...
        try:
            resp = await self._call_llm(
                provider_id=provider_id or None, prompt=prompt,
                system_prompt=self._generator_prompt(), image_urls=image_urls, umo=umo,
            )
    ...
    async def _call_generator_with_tools(self, prompt: str, event: AstrMessageEvent, image_urls: list = None, umo: str = "") -> Optional[str]:
        ...
        try:
            ...
            tools = None
        # 注入真实的对话参与者信息，防止 LLM 编造 user_id
        sender_name = event.get_sender_name()
        sender_id = event.get_sender_id()
        ...
        try:
            ...
            # 如果文本为空但存在工具调用结果（如发了图），返回占位符让流程继续
    ...
    async def _call_llm(self, provider_id: Optional[str], prompt: str,
        ...
        if not provider_id:
            return None
        # 框架(openai_source.py)自带图片转述机制，会自动处理不支持多模态的场景
        resp = await self.context.llm_generate(
            prompt=prompt, system_prompt=system_prompt,
            chat_provider_id=provider_id, image_urls=image_urls or None,
        )
        if resp is None:
            return None
